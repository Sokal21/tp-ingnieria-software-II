...
:- multifile(cond_check_size/3).
:- multifile(typecheck/2).
:- multifile(ignore_and_remove_dec/2).
:- multifile(ignore_dec/2).
:- multifile(remove_dec/2).
...
set_default :-
    ...
    retract_trace,                   %set default value: notrace
...
write_atomic_constr(delay(irreducible(C)&true,_)) :-
    !,
    write(irreducible(C)).
write_atomic_constr(subset(A,ris(X in B,R,F,P,FP))) :-
    A == B, X == P, R == [], FP == true,!,
    no_name(X), write(foreach(X in A,F)).
write_atomic_constr(subset(A,ris(X in B,R,F,P,FP))) :-
    A == B, X == P, FP == true,!,
    no_name(X), write(foreach(X in A,R,F,true)).
write_atomic_constr(subset(A,ris(X in B,R,F,P,FP))) :-
    A == B, X == P, FP = (FP1 & true),!,
    no_name(X), write(foreach(X in A,R,F,FP1)).
...
%%%% setlog_tc(+Goal,-OutConstraintList): 
% same as setlog/2 but Goal is a string representing a {log} goal
% and type checking is performed on the input goal
%
setlog_tc(GoalString,OutConstrLst) :-      
...
%%%% setlog(+Goal,-OutConstraintList,-Res) 
% Res = success | maybe
%
...
%%%% setlog(+Goal,+TimeOut,-OutConstraintList,-Res,+OptList): 
%%%% setlog(+Goal,+TimeOut,-OutConstraintList,-Res,+try([OptList_1,...,OptList_n])): 
...
setlog1(Goal,TimeOut,OutConstrLst,Res,OptsLst) :- !,           
    set_default_control,
    setlogTimeOut_opt(OptsLst,Goal,TimeOut,OutConstrLst,Res).

setlogTimeOut_opt(OptsLst,Goal,TimeOut,Constr,Res) :- 
    activate_opt(OptsLst),
    time_out(setlog1(Goal,Constr,Res1),TimeOut,Res2),
    check_timeout_res(Res2,Res1,Res).

activate_opt([]):- !.
activate_opt([Opt|OptsList]) :- 
    setlog_command(Opt),!,
    ssolve_command(Opt),
    activate_opt(OptsList).
activate_opt(Opt) :- !,
    throw(setlog_excpt('undefined option'(Opt))).
...
h(sbuilt) :-!,
    ...
    write('   -   delay(irreducible(G),C): to delay execution of G until C holds; '),nl,
    write('         otherwise return irreducible(G) - G, C {log} goals'), nl,
...
ssolve_command(mode(M)) :-           %% get the current solving mode (solver|prover)
    var(M),!,
    b_getval(smode,M).
ssolve_command(mode(M)) :- !,        %% change the solving mode (solver|prover)
    (M == solver -> true; M == prover),
    b_setval(smode,M).
...
ssolve_command(dec_type(ID,T)) :- !,     %% declare or consult a type
    b_getval(type_check,on), 
    dec_type(ID,T).

ssolve_command(reset_types) :- !,        %% delete all type declarations since
    b_getval(type_check,on),             %% {log} was consulted
    reset_types.                       
...
neg_constr(_A = _B,_,_) :-
    print_single_warning('***WARNING***: unimplemented form of negation').
...
%
preconditions(apply(F,X,_Y), (npfun(F) or comp({} with [X,X],F,{})), true) :-!. %i.e., X nin dom(F)
preconditions(_Z is _X mod Y, Y = 0, Y neq 0) :-!.
preconditions(applyTo(F,X,_Y), comp({} with [X,X],F,{}), true) :-!. %i.e., X nin dom(F)
...
ssolve_special(dec_p_type(_),[]) :- !,     %% dec_p_type
    \+type_checker.
...
atomic_constr(solved(_,_,_,_)) :-!.   % for internal use only
...
atomic_constr(foreach(_,_)) :-!.      % foreach(X in A,F[X])
atomic_constr(foreach(_,_,_,_)) :-!.  % foreach(X in A,[P1,P2,...],F[X],FP[X,P1,P2...])
atomic_constr(nforeach(_,_)) :-!.
atomic_constr(nforeach(_,_,_,_)) :-!.
atomic_constr(exists(_,_)) :-!.       % exists(X in A,F[X])
atomic_constr(nexists(_,_)) :-!.
atomic_constr(exists(_,_,_,_)) :-!.       % exists(X in A,[P1,P2,...],F[X],FP[X,P1,P2...])
...
sunifyXt(X,Y,[]) :-                           % X = t
    %catch(unify_with_occurs_check(X,Y),Msg,excpt_action(Msg)),!.  
...
%solved form: subset(S1,S2): smode(prover) & var(S1) & S1 \== S2 & \+nonvar_is_empty(S2) &
%                            or
%                            var(S1) & open_intv(S2)
%                            or
%                            open_intv(S1)
...
sat_sub([subset(S1,S2)|R1],R2,c,f) :-                 % subset(X,{...}), X var
    %b_getval(smode,solver),                          % rewritten in mode(solver)
    compute_sols,
    var(S1),
...
%solved form: inters(S1,S2,S3): (smode(prover) & one_var(S1,S2) or smode(solver) & var(S1) & var(S2)) &
%                               var(S3) &
...
sat_inters([inters(S1,S2,S3)|R1],R2,c,f) :-      % special case - inters(X,{.../X},S3), S3 var - smode(solver)
    %b_getval(smode,solver),
    compute_sols,
    var(S3),var(S1),
    nonvar(S2), S2 = _ with _,
...
sat_inters([inters(S1,S2,S3)|R1],R2,c,f) :-      % inters(S1,{...},S3), S3 and S1 var, S2 nonvar - smode(solver)
    %b_getval(smode,solver),
    compute_sols,
...
sat_inters([inters(S1,S2,S3)|R1],R2,c,f) :-      % inters(S1,S2,S3), S3 and S1 var, S2 any nonvar non-set aggr. - smode(solver)
    %b_getval(smode,solver),
    compute_sols,
...
sat_size([size(S,T)|R1],R2,c,f) :-                  % LEVEL 3: size(S,k), S var., k int. const.
    ...
    append(IntC,R1,R3),
    sat_step([X nin R,set(R),integer(M),size(R,M)|R3],R2,_,f).
sat_size([size(S,T)|R1],[size(S,T)|R2],Stop,f) :-   % size(S,k), S var., k nonvar   
...
sat_ran([ran(S,D)|R1],R2,c,F) :-                   % ran(S,{t}) or ran(S,int(a,a))
    var(S),
    nonvar(D), singleton(D,A), !,
...
sat_exists([exists(CE_Dom,Fo)|R1],R2,c,F) :-          % exists(D,Fo)
    sat_nforeach4([nforeach(CE_Dom,[],neg(Fo),true)|R1],R2,_,F).
sat_nexists([nexists(CE_Dom,Fo)|R1],R2,c,F) :-        % nexists(D,Fo)
    sat_foreach4([foreach(CE_Dom,[],neg(Fo),true)|R1],R2,_,F).   

sat_exists4([exists(CE_Dom,P,Fo,FP)|R1],R2,c,F) :-    % exists(D,Fo)
    sat_nforeach4([nforeach(CE_Dom,P,neg(Fo),FP)|R1],R2,_,F).   
sat_nexists4([nexists(CE_Dom,P,Fo,FP)|R1],R2,c,F) :-  % nexists(D,Fo)
...
%inference-rule:
% C1 &...& Ck & ... --+->
%     solved(C1,...,f) & D1 &...& Dn &...& Ck &... (k >= 2)
% e.g.:
% dom(S,D) &...& D neq {}  &... -+->
%     solved(dom(S,D),(var(S),var(D)),[1|L],f) & S neq {} &...& D neq {} &...
...
global_check1([C|RC],GC,Res,f) :-
   \+size_solver_on,                % (size_solver_on = false) size(S,N), var S, var N
    ...
    (M == 0 ->
          Res=[solved(size(S,N),(var(S),var(N)),2,f)|NewC]
     ;
...
global_check2([C|RC],GC,[C|NewC],F) :-                               % type_constr: nothing to do
    type_constr(C),!,
    global_check2(RC,GC,NewC,F).
global_check2([solved(C,Cond,Lvl,Mode)|RC],GC,NewC,F) :- !,          % temporaly remove 'solved' mode
...
%%%%%% list constraints       % (other_aggrs(on)) in-nin: T in X & T1 nin X (X is a list) --> T neq T1
...
global_check2_ran([C|RC],GC,AddedC,F) :-
    is_ran_l(C,L,S,R),                  % ran-size: ran(S,R) & size(S,N) --+-> ran(S,R) & size(S,N) & size(R,M) & M =< N
...
is_un_l(solved(un(X,Y,Z),_,Lev,_),Lev,X,Y,Z).
...
labeling_Q(X) :-                     
    (bb_inf([X],X,Min,_V1) ->        % if the variable X has a closed domain then use it
        true                         % otherwise use the default lower and upper bounds for
     ;                               % non-closed domains
        min_domain(A), Min is A
    ),
    (bb_inf([X],-X,NMax,_V2) ->      
        Max is -NMax
     ;
        max_domain(B), Max is B
    ),
    all_int(X,Min,Max).
...
is_foreach(foreach(D,Fo),FE4,C) :- !,            %foreach/2 ---> foreach/4
...
is_nforeach(nforeach(D,Fo),NFE4,C) :- !,    %nforeach/2 ---> nforeach/4
...
gen_type_constrs(dompf(T1,T2),[pfun(T1),set(T2)]) :- !,
     check_sorts([set(T1),set(T2)],_).
gen_type_constrs(comppf(T1,T2,T3),[pfun(T1),pfun(T2),pfun(T3)]) :- !,
     check_sorts([set(T1),set(T2),set(T3)],_).
...
gen_type_constrs(ndompf(T1,T2),SC) :- !,
     check_sorts([set(T1),set(T2)],SC).
%
gen_type_constrs(apply(T1,_,_),[delay(pfun(T1),false)]) :- !,
     check_sorts([set(T1)],_).
...
extract_vars(exists(V,B),Vars) :- %%NEW 
    list_term(V),!,
    extract_vars(B,List),
    remove_list(V,List,Vars).
extract_vars(exists(CT in D,F),Vars) :-   %%NEW
    !,
    extract_vars(exists(CT in D,[],F,true),Vars).
extract_vars(exists(CT in D,V,F,P),Vars) :-   %%NEW
    !,
    extract_vars(CT,L1),
    listunion(L1,V,LocalV),
    extract_vars(F,L2),
...
extract_vars(forall(X in Y,B),Vars) :-
    ...
    listunion(L1,L2,L),
    remove_var(X,L,Vars).
extract_vars(foreach(CT in D,V,F,P),Vars) :-   %%NEW
    !,
    extract_vars(CT,L1),
    listunion(L1,V,LocalV),
    extract_vars(F,L2),
...
rw_rules('./setlog/setlog_rules.pl').

%%% advanced size solver file (default: 'size_solver.pl')
size_solver_file('./setlog/size_solver.pl').

%%% type-checker file (default: 'setlog_tc.pl')
type_checker_file('./setlog/setlog_tc.pl').
...
default_mode(prover).             % don't compute variable substitutions if not necessary
...
%%% lower and upper bounds for non-closed integer domains
min_domain(A) :- A is -(2^32).
max_domain(A) :- A is 2^32.
